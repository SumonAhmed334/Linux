...
**Production runbook — field-verified, built to run for years**

| | |
|---|---|
| VM | `192.168.42.22` (private, no public IP, no inbound ports) |
...
| OS | Ubuntu 24.04 LTS |
| Status | Deployed and verified, August 2026 |

Every step below was executed on the real host. The gotchas in §14 are incidents that actually occurred during this build, not theoretical ones.

---

## 1. Read this first — the five things that will break your build

If you only read one section, read this one. Each of these cost real time during deployment.
...
Pangolin forwards `X-Forwarded-Proto: http` by default. The Euro-Office Document Server reads that header to build its own asset URLs, so it emits:

```
"Editor.bin": "http://eoffice.fiberathome.net/cache/files/data/..."
```

The browser is on `https://`, refuses those as mixed content, and the editor shows **"Download failed."** Server-side everything succeeds — the document is fetched, converted, and served. Nothing is logged as an error. You will chase JWT secrets, file permissions, private-IP filters, and startup races for hours.

Fix: add a custom header on **both** resources (§10.3).

### 1.2 Switch the iptables backend *before* installing Docker

Ubuntu 24.04 defaults to `iptables-nft`. If you install `docker-ce` first, dockerd starts immediately and writes its chains — including `FORWARD` policy `DROP` — into the **nft** backend. Switching to `iptables-legacy` afterwards makes Docker rebuild in legacy, but **the stale nft rules stay live in the kernel** and silently drop all container-to-container traffic.

Symptom: `PDOException: SQLSTATE[HY000] [2002] Connection timed out` on install, while `iptables-legacy -S FORWARD` looks perfect. DNS still works (resolved inside the container namespace, never crosses FORWARD), which makes it look like a database problem.

Fix: `update-alternatives` comes before `apt-get install docker-ce` (§4).

### 1.3 The Euro-Office container takes no bind mounts

The fork rebranded every path: `/var/log/euro-office/`, `/etc/euro-office/`, `/var/www/euro-office/`. Mounting the old OnlyOffice paths does nothing. Mounting the *new* ones breaks it:

- An empty bind mount over the log dir prevents supervisord creating its subdirectories → the DS never starts.
- The entrypoint **generates `local.json` itself** from environment variables, writing a `.tmp` then `mv`-ing it. A read-only mount makes that `mv` fail with `Device or resource busy` → crash loop.
- `App_Data` doesn't exist at the path the old docs suggest.

Fix: declare the service with **no `volumes:` key at all** (§7). Everything under those paths is generated at startup or regenerable cache.

### 1.4 The Document Server needs 60–90 seconds to start

With no volumes, it regenerates fonts, presentation themes, and JS caches on every start. Its nginx accepts connections long before docservice binds port 8000, so requests during that window get `502 Bad Gateway` or `connection refused` from the *upstream*.

Any check run too early will look like a hard failure. Wait for the log to show `Express server listening on port 8000` before believing a failure is real.

### 1.5 Nextcloud logs in UTC; your containers are in Dhaka

`TZ=Asia/Dhaka` governs the containers, but Nextcloud's `occ log:tail` timestamps are **UTC**. A 6-hour offset makes stale errors look current. During this build, two separate rabbit holes came from treating a `13:19:54Z` entry as new when it was 19:19:54 local — from before the container restarted.

Always convert before concluding an error is live. Better: clear the log, reproduce, then read.
...
| Timezone | `Asia/Dhaka` |
| Host | 12 vCPU / 12 GB RAM |

Nextcloud **34 = Hub 26 Spring** (released 9 June 2026, same day as Euro-Office GA). Nextcloud 33 is Hub 26 Winter — do not confuse them.

**Sizing.** The DS is the heavy component. 4 vCPU / 8 GB is the practical floor; this build has 12/12, which is comfortable. Budget roughly: DS 3–4 GB, MariaDB 3 GB, Nextcloud PHP 2 GB, Redis small.

---

## 4. OS and Docker

Order matters here — see §1.2.
...
**Verify no split backend** — this check takes five seconds and saves an hour:
...
**Do not also set the reverse-proxy values via `occ`.** They are read at runtime from the environment by the image's `reverse-proxy.config.php`. Setting them in both places creates two sources of truth that drift.

---

## 7. PHP tuning file

Sized for 12 GB / 12 cores. The image ships conservative defaults.
...
Note the `eurooffice` service has **no `volumes:` key**. That is deliberate and load-bearing — see §1.3.
...
> **If the install fails partway** (e.g. the iptables issue in §1.2), the entrypoint writes `version.php` before completing. On the next start it sees that file, assumes Nextcloud is installed, skips the install, and starts Apache against an empty database — a silent half-state. Fix the underlying problem, then wipe and retry:
> ```bash
> docker compose down
...
Baseline settings (note `background:cron`, **not** `background:job:mode`):
...
> **`network_mode: host` is mandatory.** Newt must reach `127.0.0.1:8080` and `127.0.0.1:6233`. On a bridge network, `localhost` is Newt's own namespace and both targets resolve to nothing — the classic "site Online but resource 502s". If `docker ps` shows a port for newt, host networking did not apply.

Expect in the log: `Tunnel connection to server established successfully!`, then `Started tcp proxy to 127.0.0.1:8080` and `...:6233`.

Newt needs outbound HTTPS to the gateway plus outbound WireGuard UDP (commonly 51820/21820).
...
**SSO must be off on both.** On the Nextcloud resource it breaks every desktop client, mobile app, WebDAV mount and CalDAV/CardDAV sync — none can render an interactive login page. On the Euro-Office resource it breaks the editor for everyone, because the browser loads DS assets as sub-resources that cannot be redirected through a login. For edge access control use Pangolin source-IP rules instead.

### 10.3 Custom headers — the critical step

On **both** resources, in the proxy settings **Custom Headers** field:

```
X-Forwarded-Proto: https
```

Without this the Document Server emits `http://` asset URLs and the editor fails with "Download failed" (§1.1). Nextcloud survives without it only because `OVERWRITEPROTOCOL=https` compensates — set it on both anyway.

### 10.4 Block the DS admin panel
...
Add a Pangolin path rule on the `eoffice` resource denying `/admin`. Verify:
...
`PHP_UPLOAD_LIMIT=2G` is what users see. Note Nextcloud **chunks** uploads, so Pangolin never sees a 2 GB body from the web UI or desktop client — the gateway limit only affects single-shot uploads (WebDAV, curl, some mobile apps). If Pangolin exposes a body-size setting, set it slightly **above** your PHP limit (e.g. `2200M`) so Nextcloud gives the user a real error instead of the proxy returning a bare 413.
...
Already applied via environment. **Verify, don't duplicate:**
...
**Why the split:** leave the internal URLs blank and both hops fall back to the public name, so NC→DS and DS→NC traffic leaves the VM, climbs the tunnel to Pangolin, and comes back down. Slow at best, broken if the LAN can't hairpin.

**ODF formats.** `.odt`/`.ods`/`.odp` may download instead of opening. The connector writes no format keys by default — configure them in **Administration settings → Euro-Office** (`https://ecloud.fiberathome.net/settings/admin/eurooffice`), not by guessing `occ` keys. Record the resulting key name:
...
Loopback binding means there is nothing to filter for the services. Only SSH is inbound.

```bash
sudo tee /usr/local/sbin/nc-fw.sh >/dev/null <<'EOF'
#!/usr/bin/env bash
# Nextcloud + Euro-Office VM firewall (iptables only)
# Services bind 127.0.0.1 and are published via the outbound Pangolin/Newt tunnel,
...
The unit needs `PartOf=docker.service` — Docker rebuilds `DOCKER-USER` from scratch on every daemon start, and a plain `oneshot` will not re-run:
...
sudo iptables -S DOCKER-USER      # rules still present, not a bare -N
```
...
Every entry here happened during this deployment.

| # | Symptom | Root cause | Fix |
|---|---|---|---|
| 1 | `PDOException ... Connection timed out` on install; DNS resolves fine | Split iptables backend: stale nft `FORWARD DROP` alongside legacy rules | §1.2 / §4 — switch backend before installing Docker |
| 2 | Install "succeeds" but no tables; Apache starts with no DB | Failed install left `version.php`; entrypoint skipped reinstall | Wipe `data/{nextcloud,db,redis}` and restart |
...
| 5 | Newt: `connection refused` to `127.0.0.1:6233` | Newt on bridge network, not host | `network_mode: host`; verify PORTS column is empty |
| 6 | DS logs directory empty; `/var/log/onlyoffice` unused | Fork rebranded paths to `/var/log/euro-office/` etc. | Read the real paths; don't mount them |
| 7 | DS crash-loop: `mv: ... local.json: Device or resource busy` | Entrypoint generates `local.json`; read-only mount blocks its `mv` | Remove the mount — entrypoint owns that file |
| 8 | DS won't start: `directory ... adminpanel/out.log does not exist` | Empty bind mount shadowed the log tree | Remove all bind mounts from the DS service |
| 9 | `--check` returns 502 / `connection refused` right after restart | DS needs 60–90s to regenerate fonts and caches | Wait for `Express server listening on port 8000` |
| 10 | Stale errors mistaken for live ones, twice | Nextcloud logs UTC; containers are Dhaka (+6) | Clear log, reproduce, then read |
| 11 | **"Download failed" in the editor; nothing logged as an error** | **Pangolin sent `X-Forwarded-Proto: http`; DS emitted `http://` asset URLs; browser blocked mixed content** | **`X-Forwarded-Proto: https` custom header on both resources** |
| 12 | `.odt`/`.ods` download instead of opening | ODF not in the connector's editable set | Configure in the connector's admin UI |
...
Raise verbosity when debugging (default is `WARN`):
...
Reading `getBaseUrlByConnection ... x-forwarded-proto=` in the docservice log is how incident #11 was finally identified. Set it back to `WARN` when done.
...
Scope it under **Administration → Security**.
...
### 15.3 Credential hygiene

Rotate anything that has ever appeared in a terminal transcript or chat:
...
Run this after the build and after any major change.
...
nc-occ status
nc-occ eurooffice:documentserver --check
nc-occ config:system:get trusted_proxies
```

In the browser:

- [ ] Login; lands on **Files**
- [ ] Administration → Overview shows no warnings
- [ ] `+ New → Document` opens the editor
- [ ] `.docx`, `.xlsx`, `.odt`, `.ods` all open, edit, and persist after reload
...
**Reboot test — do this before real data lands:**
...
docker system prune -f              # reclaim dangling images
sudo systemctl status nc-backup.timer
```

Check Administration → Overview for new setup warnings after any update.
...
# Nextcloud minor (34.0.3 -> 34.0.x)
sed -i 's|^NC_IMAGE=.*|NC_IMAGE=nextcloud:34.0.9-apache|' .env
...
docker inspect newt --format '{{.HostConfig.NetworkMode}}'   # confirm still host
```
...
- Majors ship ~3×/year with ~12 months support. Plan two upgrade windows a year.
- **Back up before every major.**
- MariaDB 11.4 is supported to May 2029. Move to the next LTS in its own window, never alongside a Nextcloud major.
- Read the connector's changelog before bumping Euro-Office — independent release cycles.
- After any Euro-Office upgrade, re-verify the `X-Forwarded-Proto` behaviour (§1.1); it is the thing most likely to regress silently.
...
- **Full-text search, preview generation, imaginary.** Each is another container with its own cost. Start lean.
- **S3 primary storage.** Changing this later is a migration, not a config edit. Decide before onboarding users.
- **DS log persistence.** Every bind mount on that container broke it (§14). If you need logs to survive restarts, use a named volume seeded from the image, not an empty bind mount.
...
| PHP tuning | `/docker/nextcloud/config-php/zz-tuning.ini` |
...
| DS config (in container) | `/etc/euro-office/documentserver/local.json` — **entrypoint-owned, do not mount** |
